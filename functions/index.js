  ...
  return db
    ...
    .then(snapshot => {
      ...
      return snapshot.forEach(doc => {
        const data = doc.data();
        return bcrypt.compare(req.body.password, data.password).then(match => {
          if (match)
            return res.json({
...
const userTasks = (req, res) => {
  return db
    ...
    .then(docSnap => {
      if (docSnap.exists) {
        return res.json({
          success: "Accessed user tasks list",
          tasks: docSnap.data().taskList
        });
      } else {
        return res.json({ error: "User UUID is invalid" });
      }
    });
};
...
const userGet = (req, res) => {
  if (req.path === "/tasks/") return userTasks(req, res);
  else return invalidAPI(req, res);
};
const userPost = (req, res) => {
  if (req.path === "/create/") return userCreate(req, res);
  else if (req.path === "/signin/") return userSignin(req, res);
  else return invalidAPI(req, res);
};

exports.user = functions.https.onRequest((req, res) => {
  if (req.method === "GET") return userGet(req, res);
  else if (req.method === "POST") return userPost(req, res);
  else return invalidAPI(req, res);
});
...
const taskCreate = (req, res) => {
  return db
    .collection("users")
    .doc(req.query.user)
    .update({ taskList: admin.firestore.FieldValue.arrayUnion(req.query.task) })
    .then(_res => {
      return db
        .collection("users")
        .doc(req.query.user)
...
const taskDelete = (req, res) => {
  return db
    ...
    .doc(req.query.user)
    .update({
      taskList: admin.firestore.FieldValue.arrayRemove(req.query.task)
    })
    .then(_res => {
      return db
        .collection("users")
...
exports.tasks = functions.https.onRequest((req, res) => {
  if (req.method === "GET") return taskGet(req, res);
  else if (req.method === "POST") return taskPost(req, res);
  else return invalidAPI(req, res);
});
