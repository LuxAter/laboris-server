<template>
  <article class="message" v-bind:class="{'is-danger': $store.state.responseStatus.status === 'error', 'is-info': $store.state.responseStatus.status === 'info'}">
    <div class="message-header">
      ...
      <p v-else-if="$store.state.responseStatus.status === 'info'">Info</p>
      <button class="delete" v-on:click="$store.commit('setStatus', null)"></button>
    </div>
    <div class="message-body">
      {{ $store.state.responseStatus.message }}
    </div>
  </article>
</template>
